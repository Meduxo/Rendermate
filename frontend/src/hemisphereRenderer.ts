...
export class HemisphereRenderer {
  ...
  render(slot: 0|1, grid: Grid, satPoint = 1.0, fisheyeX = 0.0, fisheyeY = 0.0, displace = false, offset = 0.01, maxHeight = 1.0, lines = false): void {
    ...
    offscreen.width = paddedCols; offscreen.height = sampledRows;
...
function buildMapping(count: number, fisheye: number): Uint16Array {
  ...
  for (let i = 0; i < count; i++) {
    ...
    carry = n - intN;
    for (let j = 0; j < intN && slot < slots; j++) mapping[slot++] = i;
  }
