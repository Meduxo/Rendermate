 ...
 */
export function render(canvas, grid, satPoint = 1.0) {
 ...
 */
export function renderBars(canvas, grid, satPoint = 1.0) {
    const rows = grid.length;
    const cols = rows > 0 ? grid[0].length : 0;
    canvas.width = cols * PIXEL_SIZE;
    canvas.height = rows * PIXEL_SIZE;
    ...
    for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
            ...
            if (barH <= 0)
                continue;
            ctx.fillStyle = `rgb(${byte},${byte},${byte})`;
            ctx.fillRect(c * PIXEL_SIZE, (r + 1) * PIXEL_SIZE - barH, PIXEL_SIZE, barH);
        }
    }
}
/**
